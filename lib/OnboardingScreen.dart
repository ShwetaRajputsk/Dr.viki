import 'package:flutter/material.dart';
import 'login.dart';
import 'home_page.dart';

class PlantApp extends StatelessWidget {
  const PlantApp({super.key});

  @override
  Widget build(BuildContext context) {
    return const WelcomeScreen();
  }
}

class WelcomeScreen extends StatelessWidget {
  const WelcomeScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF8F4EC), // Warm Sand Beige
      body: Stack(
        children: [
          // Full screen plant image
          Positioned.fill(
            child: Image.asset(
              'assets/images/top_plant.png',
              fit: BoxFit.cover,
              errorBuilder: (context, error, stackTrace) {
                // Fallback if image is not available
                return Container(
                  color: Colors.transparent,
                );
              },
            ),
          ),

          // // Bottom right succulents
          // Positioned(
          //   bottom: 0,
          //   right: 0,
          //   child: Image.asset(
          //     'assets/images/bottom_succulents.png',
          //     width: MediaQuery.of(context).size.width * 0.8,
          //     fit: BoxFit.contain,
          //     errorBuilder: (context, error, stackTrace) {
          //       // Fallback if image is not available
          //       return Container(
          //         width: MediaQuery.of(context).size.width * 0.8,
          //         height: 400,
          //         color: Colors.transparent,
          //       );
          //     },
          //   ),
          // ),

          // Center content
          Center(
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 24.0),
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  const SizedBox(height: 32),
                  // Title with colored words
                  RichText(
                    textAlign: TextAlign.center,
                    text: TextSpan(
                      style:
                          Theme.of(context).textTheme.headlineMedium?.copyWith(
                                fontFamily: 'Poppins',
                                fontWeight: FontWeight.w800,
                                color: const Color(0xFF2F2F2F), // Charcoal Grey
                              ),
                      children: [
                        const TextSpan(text: "Live "),
                        TextSpan(
                          text: "Healthier",
                          style: TextStyle(
                            color: const Color(0xFF27C2A4), // Futuristic Teal Glow
                            fontFamily: 'Poppins',
                            fontWeight: FontWeight.w800,
                          ),
                        ),
                        const TextSpan(text: ", Heal "),
                        TextSpan(
                          text: "Smarter",
                          style: TextStyle(
                            color: const Color(0xFFE76F51), // Terracotta Orange
                            fontFamily: 'Poppins',
                            fontWeight: FontWeight.w800,
                          ),
                        ),
                        const TextSpan(text: " with "),
                        TextSpan(
                          text: "Dr. ViKi",
                          style: TextStyle(
                            color: const Color(0xFFA7C7A1), // Healing Sage Green
                            fontFamily: 'Poppins',
                            fontWeight: FontWeight.w800,
                          ),
                        ),
                        const TextSpan(text: " !"),
                      ],
                    ),
                  ),
                  const SizedBox(height: 16),
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 24),
                    child: Text(
                      "Decode your body's balance, detect doshas instantly, and receive personalized wellness insights — all in one app powered by AI + AYUSH.",
                      textAlign: TextAlign.center,
                      style: Theme.of(context).textTheme.titleMedium?.copyWith(
                            color: const Color(0xFFAFAFAF), // Soft Ash
                            fontSize: 18,
                            height: 1.4,
                            fontWeight: FontWeight.w500,
                          ),
                    ),
                  ),
                  const SizedBox(height: 20),

                  // Get Started button with arrows
                  Padding(
                    padding: const EdgeInsets.only(right: 50.0, left: 90.0),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.end,
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Container(
                          decoration: BoxDecoration(
                            color: const Color(0xFFE76F51), // Terracotta Orange
                            borderRadius: BorderRadius.circular(30),
                          ),
                          child: Material(
                            color: Colors.transparent,
                            child: InkWell(
                              onTap: () {
                                Navigator.pushReplacementNamed(
                                    context, '/login');
                              },
                              borderRadius: BorderRadius.circular(30),
                              child: const Padding(
                                padding: EdgeInsets.symmetric(
                                    horizontal: 24, vertical: 12),
                                child: Text(
                                  "Get Started",
                                  style: TextStyle(
                                    color: Colors.white,
                                    fontSize: 16,
                                    fontWeight: FontWeight.w500,
                                  ),
                                ),
                              ),
                            ),
                          ),
                        ),
                        const SizedBox(width: 8),
                        // Double chevron arrows outside button
                        Stack(
                          children: const [
                            Padding(
                              padding: EdgeInsets.only(left: 10),
                              child: Icon(
                                Icons.chevron_right,
                                size: 24,
                                color: Color(0xFF27C2A4), // Futuristic Teal Glow
                              ),
                            ),
                            Icon(
                              Icons.chevron_right,
                              size: 24,
                              color: Color(0xFF27C2A4), // Futuristic Teal Glow
                            ),
                          ],
                        ),
                      ],
                    ),
                  ),
                ],
              ),
            ),
          ),
        ],
      ),
    );
  }
}

class GetStartedScreen extends StatelessWidget {
  const GetStartedScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF8F4EC), // Warm Sand Beige
      body: SafeArea(
        child: Padding(
          padding: const EdgeInsets.all(24.0),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Text(
                "Welcome to Dr. ViKi",
                style: Theme.of(context).textTheme.headlineSmall?.copyWith(
                      fontWeight: FontWeight.bold,
                      color: const Color(0xFF2F2F2F), // Charcoal Grey
                    ),
                textAlign: TextAlign.center,
              ),
              const SizedBox(height: 24),
              Text(
                "Discover the power of AI + AYUSH to decode your body's balance and receive personalized wellness insights.",
                style: Theme.of(context).textTheme.bodyLarge?.copyWith(
                      color: const Color(0xFFAFAFAF), // Soft Ash
                    ),
                textAlign: TextAlign.center,
              ),
              const SizedBox(height: 32),
              TextButton.icon(
                onPressed: () {
                  Navigator.pop(context);
                },
                icon: const Icon(Icons.chevron_left, color: Color(0xFFA7C7A1)), // Healing Sage Green
                label: const Text(
                  "Back to home",
                  style: TextStyle(color: Color(0xFFA7C7A1)), // Healing Sage Green
                ),
              ),
            ],
          ),
        ),
      ),
    );
  }
}
